...
// UP-Chiapas  I.D.S.
// Completado el 28 de Junio de 2017, en 3 días.
...
int main(){
	...
	int variableSelectivaRH; // Nos sirve para saber si en la columna de palabras se debe poner R ó S
	...
	system("title  MÉTODO SIMPLEX AVANZADO");
	opcionMenu= implementarModuloDeMenu(); // Impresión de Menú de objetivos
	
	if(opcionMenu != 3){
		
		cout<<" MÉTODO SIMPLEX AVANZADO"<< endl;
		cout<<"-----------------------"<< endl<< endl<< endl;
		...
		// Creación de la matriz simplex, vectores y otras variables
		...
		for(int i=0; i<numeroDeFilas; i++){
			...
			llenadoDeLaVariableX(arregloDeInsercionDeValores, numeroDeVariables, &banderaDeLlenadoDePrimeraVez);
	 		...
	 		if(variableSelectivaRH == 2 || variableSelectivaRH == 3)
	 			arrayIndicesR.push_back(i);	// Nos sirve para saber en que posición estan las R
		...
		MessageBox(NULL,"Método Simplex abortado","Matriz Simplex", MB_OK | MB_ICONINFORMATION | MB_DEFBUTTON1);
	getch();
	return 0;
}
...
void llenadoDeLaVariableH(vector <double> &arregloDeInsercionDeValores, int numeroDeMayorIguales, bool *banderaDeLlenadoDePrimeraVez, int variableSelectivaRH){
	...
	bool numeroNegativo;
	static int indiceDeLlenadoDeH= 0; // Nos ayuda a saber en que lugar debemos poner 0 y donde un 1 automáticamente
...
void llenadoDeLaVariableR(vector <double> &arregloDeInsercionDeValores, int sumaDeR, bool *banderaDeLlenadoDePrimeraVez, int variableSelectivaRH, double variableGranM){
	...
	bool numeroNegativo;
	static int indiceDeLlenadoDeR= 0; // Nos ayuda a saber en que lugar debemos poner 0 y donde un 1 automáticamente
...
void llenadoDelLadoDerechoDeLaEcuacion(vector <double> &arregloDeInsercionDeValores, bool *banderaDeLlenadoDePrimeraVez){
	...
	if(*banderaDeLlenadoDePrimeraVez == true)
		...
		do{
			cout<<"\n\n Ingrese el valor derecho de la ecuación: ";
			cin>> valorUsuario;
...
int encontrarCriterioDeOptimalidadSimplex(vector < vector <double> > &matrizSimplex, int numeroDeColumnas){
	...
	// El criterio de optimalidad consiste en encontrar el numero mayor negativo de la fila Z únicamente, excepto el coheficiente de la columna solución
...
int implementarModuloDeMenu(){
	...
	char teclaOprimida;
	string opcionesMenu[]={"1. Maximizar","2. Minimizar","3. Salir del menú"};
...
void imprimirMenuDeRestricciones(string opcionesMenu[], int posicionActual, int numeroDeOpciones, int indiceI){
	...
	cout<<" MÉTODO SIMPLEX AVANZADO"<< endl;
	cout<<"-----------------------"<< endl<< endl<< endl;
	cout<<" Seleccione el tipo de operador lógico de la restrcción "<< indiceI<< ": "<< endl;
	for(int i=0; i < numeroDeOpciones; i++){
		if(posicionActual == i){
			highvideo();
			textbackground(GREEN);
...
void corregirTablaDeMalComportamiento(vector < vector <double> > &matrizSimplex, vector <double> &arrayIndicesR, int opcionMenu, double variableGranM, int numeroDeColumnas){
	...
	for(int i=0; i<1; i++){
		for(int j=0; j<numeroDeColumnas; j++){
			...
			sumaDeVariablesR= 0;
			if(opcionMenu == 1)
				matrizSimplex[i][j]= matrizSimplex[i][j] - resultadoR;
			else
				matrizSimplex[i][j]= matrizSimplex[i][j] + resultadoR;
		}
	}
